...
class NewsTableViewController: UITableViewController{
    ...
    @objc func refreshNews(){
        ...
        //MARK: Паттерн Adapter сюда приходят данные через замыкание от NetworkNews ( сетевого запроса)
    ...
    override func viewDidLoad() {
        ...
        let networkNewsProxy = NetworkNewsServiceProxy(networkNews: self.networkNews)

        networkNewsProxy.networkNews.newsRequest {[weak self]  news, nextFrom in
            guard let self = self else {return}
        ...
        networkNewsProxy.newsRequest { news, nextFrom in }
    ...
    //MARK: simple Factory. Модель config передается в ячейку celltitle,cellText,cellImage и cellInfo
